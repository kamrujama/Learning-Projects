define(["knockout", "koUtil", "autho", "knockout.validation", "jQuery.base64", "constants", "globalFunction", "appEnum", "chosen"], function (ko, koUtil, autho) {
    ...
    return function getParameterValuesForResult() {
        ...
        $('#paramValueForTempResultHeader').mouseup(function () {
            $("#paramValueForTempResult").draggable({ disabled: true });
        });

        $('#paramValueForTempResultHeader').mousedown(function () {
            $("#paramValueForTempResult").draggable({ disabled: false });
        });
        ...
        self.moveLeft = function () {
            ...
            left = moveTabsLeft("#templateTabs", "#movePVTLeft", "#movePVTRight", self.templateXML().length, left, contrWidth);
        }

        self.moveRight = function () {
			contrWidth = $('#resultSectionTemplate').width();        //When we move right
            left = moveTabsRight("#templateTabs", "#movePVTLeft", "#movePVTRight", self.templateXML().length, left, contrWidth);
        }
        ...
        function getDeviceFormFiles(JsonXmlData, templateXML) {
            ...
            if (koUtil.isDeviceProfile() == true) {
                var uniqueID = getSelectedUniqueId('Devicejqxgrid');
                getDeviceFormFileReq.UniqueDeviceId = uniqueID[0];
                getDeviceFormFileReq.DeviceSearch = null;
                getDeviceFormFileReq.ColumnFilter = null;

                Selector.SelectedItemIds = uniqueID[0];
        ...
        function getGetParameterValuesForTemplateDetails(templateXML) {
            ...
            getParameterValuesForTemplateReq.TemplateId = koUtil.deviceViewParameterTemplate.TemplateId;
            ...
            if (source != '') {
                ...
                if (Param[l].ValueType.Type == 'String') {
                    ...
                    if (Param[l].PartialMask && Param[l].PartialMask.toUpperCase() == 'TRUE') {
                        ...
                        $(id).val(source[0].ParamValue);
                        if (Param[l].MaskLength && $(id).val().length > Param[l].MaskLength) {
                            if (Param[l].MaskDirection && Param[l].MaskDirection.toUpperCase() == AppConstants.get('PARTIAL_MASKING_DIRECTION_RIGHT')) {
                                var lastcharcters = $(id).val().substr($(id).val().length - Param[l].MaskLength);
                                ...
                                var Firstcharcters = $(id).val().substr(0, Param[l].MaskLength);
                        ...
                        else if (Param[l].MaskLength && $(id).val().length <= Param[l].MaskLength) {
                            $(id).val('********');
                        }
                        else if ($(id).val().length > 4) {
                            var lastcharcters = $(id).val().substr($(id).val().length - 4);
                            $(id).val('****' + lastcharcters);
