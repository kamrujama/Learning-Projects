define(["knockout", "koUtil", "autho", "advancedSearchUtil", "knockout.validation", "jQuery.base64", "constants", "globalFunction", "appEnum", "chosen"], function (ko, koUtil, autho, ADSearchUtil) {
    ...
    return function getDetailsOfEditTemplate() {
        ...
        //alert(JSON.stringify(self.templateXML()))
        ...
        $('#editTempVpdxHeader').mouseup(function () {
            $("#editTempVpdx").draggable({ disabled: true });
        });

        $('#editTempVpdxHeader').mousedown(function () {
            $("#editTempVpdx").draggable({ disabled: false });
        });
        ...
        self.moveLeft = function () {
			contrWidth = $('#resultSectionDeviceProfileEditTemplate').width();        //When we move left
            left = moveTabsLeft("#templateTabs", "#moveLeft", "#moveRight", self.templateXML().length, left, contrWidth);
        }

        self.moveRight = function () {
			contrWidth = $('#resultSectionDeviceProfileEditTemplate').width();        //When we move right
            ...
            if (data) {
                if (data.responseStatus.StatusCode == AppConstants.get('SUCCESS')) {
                    ...
                    if (data.getDeviceFormFileResp) {
                        ...
                        if (!data.getDeviceFormFileResp.ParameterFormFiles || data.getDeviceFormFileResp.ParameterFormFiles.length < 1)
                            return
                        ...
                        for (var i = 0; i < JsonXmlData1.length; i++) {
                            ...
                            for (var j = 0; j < JsonXmlData1[i].ParameterFile.length; j++) {
                                ...
                                //('paramDataArray==' + paramDataArray.length)
                        ...
                        setTimeout(function () {
                            contrWidth = $('#resultSectionDeviceProfileEditTemplate').width();
                            left = 0;
